"""NIFTY Glucksman option-buying bot with live alerts and premium backtests.

What this program does
- Monitors NIFTY 50 on 5-minute candles.
- Uses an OPS-style ensemble inspired by Glucksman-Lahanis
  (FTRL momentum + PAMR/CWMR mean reversion + OGU meta weighting).
- Sends Telegram alerts when the full Glucksman setup appears.
...
- EXPIRED_OPTIONS_EXPIRY_CODE=1
- OPS_FTRL_LOOKBACK=8
- OPS_CWMR_LOOKBACK=20
- OPS_META_ETA=0.08
- OPS_SIGNAL_THRESHOLD=0.20
- OPS_EXIT_FLIP_THRESHOLD=0.15
- NO_NEW_ENTRY_AFTER=14:45
- BACKTEST_ENTRY_SLIPPAGE=0.00
- BACKTEST_EXIT_SLIPPAGE=0.00
...
def _env_time(name: str, default: dt.time) -> dt.time:
    ...
    raw = raw.strip()
    try:
        parsed = dt.datetime.strptime(raw, "%H:%M").time()
        return parsed
    except Exception:
        return default
...
class Config:
    ...
    no_new_entry_after: dt.time = dt.time(14, 45)
    ...
    expired_options_expiry_code: int = 1
    ops_ftrl_lookback: int = 8
    ops_cwmr_lookback: int = 20
    ops_meta_eta: float = 0.08
    ops_signal_threshold: float = 0.20
    ops_exit_flip_threshold: float = 0.15
    ...
    def from_env() -> "Config":
        ...
        return Config(
            ...
            no_new_entry_after=_env_time("NO_NEW_ENTRY_AFTER", dt.time(14, 45)),
            ...
            expired_options_expiry_code=_env_int("EXPIRED_OPTIONS_EXPIRY_CODE", 1),
            ops_ftrl_lookback=_env_int("OPS_FTRL_LOOKBACK", 8),
            ops_cwmr_lookback=_env_int("OPS_CWMR_LOOKBACK", 20),
            ops_meta_eta=_env_float("OPS_META_ETA", 0.08),
            ops_signal_threshold=_env_float("OPS_SIGNAL_THRESHOLD", 0.20),
            ops_exit_flip_threshold=_env_float("OPS_EXIT_FLIP_THRESHOLD", 0.15),
...
class DhanApiClient:
    ...
    def rolling_option_intraday(
        ...
        expiry_code = int(self.cfg.expired_options_expiry_code)
        if expiry_code <= 0:
            expiry_code = 1
        payload = {
...
def add_indicators(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # Paper-inspired OPS features: FTRL momentum + PAMR/CWMR-style mean reversion.
    ret = out["close"].astype(float).pct_change().fillna(0.0)
    ftrl_mom = ret.rolling(cfg.ops_ftrl_lookback, min_periods=max(2, cfg.ops_ftrl_lookback // 2)).mean().fillna(0.0)
    ftrl_sig = np.sign(ftrl_mom).astype(float)

    pamr_raw = -(ret - 0.0005)
    pamr_sig = np.sign(pamr_raw).astype(float)

    cwmr_vol = ret.rolling(cfg.ops_cwmr_lookback, min_periods=max(3, cfg.ops_cwmr_lookback // 2)).std().replace(0.0, np.nan)
    cwmr_conf = (-ret / (cwmr_vol + 1e-9)).clip(-3.0, 3.0).fillna(0.0)
    cwmr_sig = np.sign(cwmr_conf).astype(float)
    ...
    for i in range(len(out)):
        if i > 0:
            prev_sigs = np.array([ftrl_sig.iloc[i - 1], pamr_sig.iloc[i - 1], cwmr_sig.iloc[i - 1]], dtype=float)
            expert_ret = prev_sigs * float(ret.iloc[i])
            w = w * np.exp(cfg.ops_meta_eta * expert_ret)
            w_sum = float(w.sum())
        ...
        now_sigs = np.array([ftrl_sig.iloc[i], pamr_sig.iloc[i], cwmr_sig.iloc[i]], dtype=float)
        meta_scores.append(float(np.dot(w, now_sigs)))
        w_ftrl.append(float(w[0]))
        w_pamr.append(float(w[1]))
        w_cwmr.append(float(w[2]))

    out["ops_return"] = ret
    out["ops_ftrl_signal"] = ftrl_sig
    out["ops_pamr_signal"] = pamr_sig
    out["ops_cwmr_signal"] = cwmr_sig
    out["ops_cwmr_conf"] = cwmr_conf
    out["ops_weight_ftrl"] = w_ftrl
    out["ops_weight_pamr"] = w_pamr
    out["ops_weight_cwmr"] = w_cwmr
    out["ops_meta_score"] = meta_scores
...
def build_Glucksman_signal(df: pd.DataFrame, idx: int, cfg: Config) -> Optional[GlucksmanSignal]:
    if idx < max(cfg.supertrend_period + 2, cfg.ops_cwmr_lookback, cfg.volume_lookback):
        return None

    row = df.iloc[idx]
    if pd.isna(row.supertrend) or pd.isna(row.macd) or pd.isna(row.macd_signal) or pd.isna(row.vwap) or pd.isna(row.ops_meta_score):
        return None
    ...
    if row.timestamp < market_open + dt.timedelta(minutes=cfg.avoid_first_minutes):
        return None
    if row.timestamp.time() >= cfg.no_new_entry_after:
        return None
    ...
    score = float(row.ops_meta_score)
    if abs(score) < cfg.ops_signal_threshold:
        return None

    ftrl_sig = int(float(row.ops_ftrl_signal))
    pamr_sig = int(float(row.ops_pamr_signal))
    cwmr_sig = int(float(row.ops_cwmr_signal))
    base_reasons = [
        "OPS ensemble inspired by Glucksman-Lahanis framework (FTRL, PAMR, CWMR + OGU weighting)",
        f"Meta score: {score:.3f} (threshold {cfg.ops_signal_threshold:.2f})",
        f"Expert signs: FTRL={ftrl_sig:+d}, PAMR={pamr_sig:+d}, CWMR={cwmr_sig:+d}",
        f"Meta weights: FTRL={float(row.ops_weight_ftrl):.2f}, PAMR={float(row.ops_weight_pamr):.2f}, CWMR={float(row.ops_weight_cwmr):.2f}",
    ...
    bullish_conditions = [
        score >= cfg.ops_signal_threshold,
    ...
    if all(bullish_conditions):
        trigger_key = f"CE|OPS|{row.timestamp}|{score:.3f}"
        reasons = [
            "OPS ensemble bullish alignment",
    ...
    if all(bearish_conditions):
        trigger_key = f"PE|OPS|{row.timestamp}|{score:.3f}"
        reasons = [
            "OPS ensemble bearish alignment",
...
def opposite_exit_signal(df: pd.DataFrame, idx: int, side: str, threshold: float = 0.15) -> bool:
    if idx <= 0:
        return False
    row = df.iloc[idx]
    if "ops_meta_score" in df.columns and not pd.isna(row.get("ops_meta_score", np.nan)):
        score = float(row.ops_meta_score)
        if side == "CE":
...
class GlucksmanBacktester:
    ...
    def run(self, start_date: str, end_date: str) -> BacktestResult:
        ...
        for idx in range(len(index_df) - 1):
            ...
            signal = build_Glucksman_signal(index_df, idx, self.cfg)
            if signal is None:
                continue
            ...
            signal_time = signal_row.timestamp.to_pydatetime() if hasattr(signal_row.timestamp, "to_pydatetime") else signal_row.timestamp

            # Intraday-only execution hygiene:
            # 1) do not carry a signal to the next session open
            # 2) do not open a fresh position at/after configured square-off time
            if entry_time.date() != signal_time.date():
                last_trigger_key = signal.trigger_key
                continue
            if entry_time.time() >= self.cfg.square_off_time:
                last_trigger_key = signal.trigger_key
                continue
    ...
    def _simulate_option_trade(
        ...
        entry_fill = max(0.05, entry + max(0.0, self.cfg.backtest_entry_slippage))
        ...
        for pos in range(entry_pos, len(option_df)):
            ...
            if not t1_hit and high >= t1:
                t1_fill = max(0.05, t1 - max(0.0, self.cfg.backtest_exit_slippage))
                realized += half_qty * (t1_fill - entry_fill)
            ...
            if idx is not None and idx > entry_idx and opposite_exit_signal(index_df, idx, signal.side, self.cfg.ops_exit_flip_threshold):
                exit_price = close
                exit_reason = "opposite signal"
                break

        exit_fill = max(0.05, exit_price - max(0.0, self.cfg.backtest_exit_slippage))
        realized += remaining_qty * (exit_fill - entry_fill)
...
def format_signal(signal: GlucksmanSignal) -> str:
...
HELP_TEXT = """<b>NIFTY Glucksman Bot Commands</b>
...
Strategy:
OPS ensemble (FTRL + PAMR + CWMR with OGU-style meta weighting) on 5-minute NIFTY candles.
...
class GlucksmanBot:
    ...
    def build_live_plan(self, signal: GlucksmanSignal, index_df: pd.DataFrame, signal_idx: int) -> GlucksmanSignal:
    ...
    def open_live_position(self, signal: GlucksmanSignal) -> None:
    ...
    def track_live_position(self, index_df: pd.DataFrame, position: LivePosition) -> bool:
        ...
        for _, row in new_rows.iterrows():
            ...
            if idx is not None and pd.Timestamp(index_df.iloc[idx].timestamp) > pd.Timestamp(position.signal_time):
                if opposite_exit_signal(index_df, idx, plan.side, self.cfg.ops_exit_flip_threshold):
                    action = "EXIT FULL" if not position.t1_hit else "EXIT REMAINING"
                    self.bot.send(
                        format_live_position_update(
                            position,
                            action,
                            "opposite Glucksman signal",
                            close,
                            row.timestamp,
    ...
    def live_check(self) -> None:
        ...
        signal = build_Glucksman_signal(index_df, len(index_df) - 1, self.cfg)
        if signal is None:
            return
    ...
    def scan_worker(self, start_date: str, end_date: str) -> None:
        try:
            ...
            backtester = GlucksmanBacktester(self.cfg, self.api, self.resolver, self.scan_stop_event)
    ...
    def run(self) -> None:
        print(f"Glucksman Bot started | {NIFTY50_NAME}")
        self.bot.send(
            f"<b>{NIFTY50_NAME} Glucksman Bot is online</b>\n\n"
        ...
        while True:
            try:
                ...
                self.bot.send(f"{NIFTY50_NAME} Glucksman Bot stopped.")
                print("Stopped.")
                return
